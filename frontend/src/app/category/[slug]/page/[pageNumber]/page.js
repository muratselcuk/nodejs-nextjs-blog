...
export default async function CategoryPaginatedPage({ params }) {
  ...
  return (
    <main className="max-w-3xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">Kategori: {currentPosts[0]?.category?.name}</h1>
  ...
  {currentPosts.map((post) => {
    ...
    <li
      key={post.id}
      className="bg-white rounded-xl shadow-md hover:shadow-lg hover:border-blue-500 border border-transparent transition p-5"
    >
      <Image
                src={imageUrl}
                alt={post.title}
      ...
      <div className="flex justify-between items-center mt-10 text-sm">
        {page > 1 ? (
          <Link href={`/category/${slug}/page/${page - 1}`} className="text-blue-600 hover:underline">
            ← Önceki
          </Link>
        ) : <div />}

        {page < totalPages ? (
          <Link href={`/category/${slug}/page/${page + 1}`} className="text-blue-600 hover:underline ml-auto">
            Sonraki →
          </Link>
        ) : <div />}
      </div>
